...
internal static class PathingService
{
    ...
    internal static SvgPointF Reflect(SvgPointF point, SvgPointF mirror)
    {
    ...
    internal static SvgPointF ToAbsolute(SvgPointF point, bool isRelative, SvgPointF start)
    {
    ...
    internal static SKPath ToPath(this SvgPathSegmentList svgPathSegmentList, SvgFillRule svgFillRule)
    {
        ...
        var start = SvgPointF.Empty;
        var prevMove = start;
        var points = new List<SvgPointF>();
                ...
                case SvgCubicCurveSegment svgCubicCurveSegment:
                    {
                        ...
                        if (float.IsNaN(firstControlPoint.X) || float.IsNaN(firstControlPoint.Y))
                        {
                            ...
                            if (prev >= 0 && svgPathSegmentList[prev] is SvgCubicCurveSegment)
                            {
                                ...
                                firstControlPoint = Reflect(new SvgPointF(prevSecondControlPoint.X, prevSecondControlPoint.Y), start);
                            }
                            else
                            {
                ...
                case SvgQuadraticCurveSegment svgQuadraticCurveSegment:
                    {
                        ...
                        if (float.IsNaN(controlPoint.X) || float.IsNaN(controlPoint.Y))
                        {
                            ...
                            if (prev >= 0 && svgPathSegmentList[prev] is SvgQuadraticCurveSegment)
                            {
                                ...
                                controlPoint = Reflect(new SvgPointF(prevControlPoint.X, prevControlPoint.Y), start);
                            }
                            else
                            {
                                controlPoint = start;
