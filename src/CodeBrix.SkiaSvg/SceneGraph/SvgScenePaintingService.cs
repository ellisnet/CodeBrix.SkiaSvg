...
internal static class SvgScenePaintingService
{
        ...
        DrawAttributes ignoreAttributes)
    {
        ...
        switch (server)
        {
            case SvgColorServer SvgColorServer:
                return TryApplyColor(svgVisualElement, SvgColorServer, opacity, skPaint, ignoreAttributes);

            case SvgPatternServer svgPatternServer:
                {
                    var colorInterpolation = GetColorInterpolation(svgVisualElement);
                    var isLinearRgb = colorInterpolation == SvgColorInterpolation.LinearRGB;
    ...
    private static bool TryApplyColor(
        SvgVisualElement svgVisualElement,
        SvgColorServer SvgColorServer,
        float opacity,
        SKPaint skPaint,
        DrawAttributes ignoreAttributes)
    {
        var skColor = GetColor(SvgColorServer, opacity, ignoreAttributes);
        var colorInterpolation = GetColorInterpolation(svgVisualElement);
        var isLinearRgb = colorInterpolation == SvgColorInterpolation.LinearRGB;

        if (colorInterpolation == SvgColorInterpolation.SRGB)
        {
    ...
    {
        if (fallbackServer is not SvgColorServer SvgColorServerFallback)
        {
            return false;
        }

        var skColor = GetColor(SvgColorServerFallback, opacity, ignoreAttributes);
    ...
    private static SKColor GetColor(SvgColorServer SvgColorServer, float opacity, DrawAttributes ignoreAttributes)
    {
        var colour = SvgColorServer.ColorValue;
        var alpha = ignoreAttributes.HasFlag(DrawAttributes.Opacity)
            ? SvgColorServer.ColorValue.A
            : CombineWithOpacity(SvgColorServer.ColorValue.A, opacity);
    ...
    private static SvgColorInterpolation GetColorInterpolation(SvgElement svgElement)
    {
        return svgElement.ColorInterpolation switch
        {
            SvgColorInterpolation.Auto => SvgColorInterpolation.SRGB,
            SvgColorInterpolation.LinearRGB => SvgColorInterpolation.LinearRGB,
            _ => SvgColorInterpolation.SRGB,
        };
    }
        ...
        bool isLinearRgb)
    {
        foreach (var svgReferencedGradientServer in svgReferencedGradientServers)
        {
            ...
            foreach (var child in svgReferencedGradientServer.Children)
            {
                ...
                if (server is not SvgColorServer stopColorSvgColorServer)
                {
                    continue;
                }

                var stopOpacity = AdjustSvgOpacity(svgGradientStop.StopOpacity);
                var stopColor = GetColor(stopColorSvgColorServer, opacity * stopOpacity, ignoreAttributes);
                if (isLinearRgb)
                {
