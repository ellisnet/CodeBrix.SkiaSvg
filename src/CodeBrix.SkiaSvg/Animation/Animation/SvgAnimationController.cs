...
public sealed class SvgAnimationController : IDisposable
{
    ...
    private static bool TryApplyAccumulation(AnimationBinding binding, SvgAnimationValueElement animation, IReadOnlyList<string> values, AnimationSample sample, bool forceColorInterpolation, ref string value)
    {
        ...
        {
            value = new SvgColorServer(SvgColor.FromArgb(
    ...
    private static bool TryResolveTypedPacedDistance(object fromObject, object toObject, out float distance)
    {
        ...
        switch (fromObject)
        {
            ...
            case SvgPaintServer fromPaint when toObject is SvgPaintServer toPaint:
                ...
                return false;
            case SvgColorServer fromColour when toObject is SvgColorServer toColour:
                distance = ResolveColorDistance(fromColour.ColorValue, toColour.ColorValue);
                return true;
            default:
                return false;
        }
    ...
    private static float ResolveColorDistance(SvgColor fromColor, SvgColor toColor)
    {
    ...
    private static bool TryInterpolateTypedValue(object fromObject, object toObject, float progress, out string result)
    {
        ...
        switch (fromObject)
        {
            ...
            case SvgColorServer fromColour when toObject is SvgColorServer toColour:
                return TryInterpolateColor(fromColour.ColorValue, toColour.ColorValue, progress, out result);
            default:
                return false;
        }
    }
    ...
    private static bool TryInterpolateColor(SvgColor fromColor, SvgColor toColor, float progress, out string result)
    {
        var color = SvgColor.FromArgb(
        ...
        result = new SvgColorServer(color).ToString();
        return true;
    }
    ...
    private static bool TryGetColor(string value, out SvgColor color)
    {
        color = default;
    ...
    private static bool TryGetColor(SvgPaintServer paintServer, out SvgColor color)
    {
        if (paintServer is SvgColorServer colourServer)
        {
            color = colourServer.ColorValue;
            return true;
        }
    ...
    private static bool TryAddValue(AnimationBinding binding, string baseValue, string byValue, out string result)
    {
        ...
        {
            result = new SvgColorServer(SvgColor.FromArgb(
