...
public class SvgAnimationControllerTests
{
    ...
    public void CreateAnimatedDocument_AppliesCoreAnimationTypes()
    {
        ...
        var fill = Assert.IsType<SvgColorServer>(color!.Fill);
        Assert.Equal((byte)0, fill.ColorValue.R);
        Assert.InRange(fill.ColorValue.G, (byte)127, (byte)128);
        Assert.InRange(fill.ColorValue.B, (byte)127, (byte)128);
    ...
    public void CreateAnimatedDocument_AppliesInheritedCssAnimationsWhenWhitespaceCssParameterIsProvided()
    {
        ...
        var fill = Assert.IsType<SvgColorServer>(root.Fill);
        Assert.Equal((byte)0, fill.ColorValue.R);
        Assert.InRange(fill.ColorValue.G, (byte)84, (byte)86);
        Assert.InRange(fill.ColorValue.B, (byte)127, (byte)128);
    }
    ...
    {
        return Path.GetFullPath(Path.Combine("Assets", "Svg", name));
    }
    ...
    private static System.Collections.Generic.List<CodeBrix.SkiaSvg.ShimSkiaSharp.SKPicture> GetAnimatedSubtreePictures(SKSvg svg)
    {
    ...
    private static System.Collections.Generic.List<CodeBrix.SkiaSvg.ShimSkiaSharp.SKPicture> CollectLeafPictures(CodeBrix.SkiaSvg.ShimSkiaSharp.SKPicture picture)
    {
        var nestedPictures = picture.Commands!
            ...
            .Cast<CodeBrix.SkiaSvg.ShimSkiaSharp.SKPicture>()
            .ToList();

        if (nestedPictures.Count == 0)
        {
            return new System.Collections.Generic.List<CodeBrix.SkiaSvg.ShimSkiaSharp.SKPicture> { picture };
        }
    ...
    private static string GetPictureSignature(CodeBrix.SkiaSvg.ShimSkiaSharp.SKPicture picture)
    {
    ...
    private static void AppendPictureSignature(StringBuilder builder, CodeBrix.SkiaSvg.ShimSkiaSharp.SKPicture picture)
    {
        builder
            .Append('[')
