...
public class VectorDrawableTests
{
    ...
    public void FromVectorDrawable_MapsRootSizeOpacityAndPaintAlpha()
    {
        ...
        var fill = Assert.IsType<SvgColorServer>(path.Fill);
        var stroke = Assert.IsType<SvgColorServer>(path.Stroke);

        Assert.Equal(255, fill.ColorValue.A);
        Assert.Equal(255, stroke.ColorValue.A);
